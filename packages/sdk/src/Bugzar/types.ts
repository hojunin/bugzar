...
export interface BugzarProps {
  ...
  /**
   * Bugzar backend base URL (e.g. `https://your-bugzar-backend.example.com`),
   * or `{ url, headers? }`. The backend is the **Jira backend only** (auth + AI
   * draft + issue creation); set it together with `jira` to enable the review
   * drawer. Web sharing is via `onExport` → your storage, not the backend.
   */
  endpoint?: Endpoint;
