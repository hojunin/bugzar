// Vendored from react-grab @ 4ab9d7d.
// Modifications: inlined MAX_SELECTOR_COMBINATIONS + replaced 3 imported
// error classes with local named Error subclasses so this file is standalone
// (no other behavioral changes — the heuristic is verbatim).

const MAX_SELECTOR_COMBINATIONS = 10_000;

class NonElementNodeError extends Error {
  constructor() {
...
const collectCombinations = (
  ...
  const head = stack[0];
  if (!head) return [currentPath];
  for (const selectorNode of head) {
