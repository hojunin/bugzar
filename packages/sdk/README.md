...
only the public `clientId` is a prop; the token exchange runs in the Worker.
...
1. In the Atlassian developer console, create an OAuth 2.0 (3LO) app with the Jira
   scopes and register the redirect URI:
   `https://bugzar-backend.<your-subdomain>.workers.dev/oauth/callback`
2. Put the credentials on the Worker:
   `wrangler secret put ATLASSIAN_CLIENT_ID` and
