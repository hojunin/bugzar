...
The viewer ships **with the Worker** — there is no separate deploy:

```sh
pnpm --filter @bugzar/backend run deploy
```

`predeploy` builds the viewer with `VITE_BASE=/v/` into `packages/backend/public/v`,
then `wrangler deploy` uploads the Worker + the viewer assets in one shot. Security
headers for the viewer live in `packages/backend/public/_headers` (CSP, X-Frame-
Options, etc., scoped to `/v/*`).

### SDK "View replay"

No viewer config — just set `endpoint`. The Worker's `reportUrl` (`<endpoint>/r/<id>`)
redirects to the viewer it serves:

```tsx
<Bugzar endpoint="https://bugzar-backend.<sub>.workers.dev" />
```
