...
describe('input masking (#46 — fail-safe default)', () => {
  ...
  it('captureSnapshot masks every input by default (fail-safe)', () => {
    captureSnapshot();
  ...
  it('startRecording masks every input by default (fail-safe)', () => {
    startRecording({ onBatch: vi.fn(), batchIntervalMs: 1000 });
    expect(lastRecordOpts?.maskAllInputs).toBe(true);
    expect(lastRecordOpts?.maskInputOptions).toBeUndefined();
    stopRecording(); // clear module stopFn so the idempotent guard can't leak
  });

  it('opting out (maskAllInputs:false) still masks password fields', () => {
    captureSnapshot('.bugzar-root', false, false);
