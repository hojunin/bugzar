...
/**
 * Default per-type input masking. password is always masked; we also
 * mask the field types most commonly carrying PII (number can be credit
 * cards, search isn't usually sensitive but DevTools defaults to masking
 * autocomplete=cc-* anyway). rrweb merges this with maskAllInputs at
 * runtime: when maskAllInputs is true, every type is masked regardless.
 */
...
export const startRecording = ({
  ...
  stopFn =
    record({
      ...
      // When the opt-in "mask everything" flag is on, rrweb masks every
      // input. When off (default), only the types listed in
      // SENSITIVE_INPUT_OPTIONS get masked — password etc. Regular text
      // inputs preserve their visible value so reviewers can read what
      // was actually typed during reproduction.
      maskAllInputs,
 ...
 *
 * Masking mirrors `startRecording`: password inputs are masked by default and
 * `maskAllInputs` masks every field — without this the design-pick snapshot would
 * capture on-page credentials in cleartext into a report uploaded to R2 / Jira.
 */
